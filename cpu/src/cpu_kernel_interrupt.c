	...
	while(continuar){
		...
		switch(cod_op){
			case INTERRUPT_QUANTUM:
				...
				if(!dictionary_has_key(diccionario_interrpuciones,string_itoa(pid_interrupcion))){
					dictionary_put(diccionario_interrpuciones,string_itoa(pid_interrupcion),&cod_op);
				}
